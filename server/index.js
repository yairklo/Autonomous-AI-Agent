...
app.post('/api/chat/sync', async (req, res) => {
  ...
  const waCvSync = config.autoSubmitWhatsappCv ? detectWhatsappCvSubmit(text) : null;
  if (waCvSync) {
    try {
      const logs = [];
      const mcpArgs = await resolveCvSubmitArgs(waCvSync, {
        onLog: (line) => { console.log(line); logs.push(line); },
      });
      ...
      return res.json({
        clientId,
        sessionId: null,
        text: formatWhatsappCvSubmitSummary(mcpResult),
...
function formatWhatsappJobScanSummary(mcpResult) {
  ...
  const lines = jobs.slice(0, 8).map((j, i) => {
    ...
    const contact = j.contacts?.emails?.[0] || j.contacts?.phones?.[0] || j.contacts?.urls?.[0];
    const contactBit = contact ? ` → ${contact}` : '';
    return `${i + 1}. [${j.groupName}] ${j.author}${when}${contactBit}: ${j.snippet}`;
  });
  return `${header}\n${lines.join('\n')}\nSay "הגש קו״ח" / "submit CV" to draft an application.`;
}

async function resolveCvSubmitArgs(waCv, { onLog } = {}) {
  const args = { ...(waCv.mcpArgs || {}) };
  if (!waCv.resolveFromScan || args.jobText || args.recipientEmail) return args;
  onLog?.('[mcp] submit_whatsapp_job_cv resolving job via scan_whatsapp_jobs…');
  const scan = await executeMcpTool('scan_whatsapp_jobs', { exportPath: config.whatsappExportsDir, limit: 20 }, { onLog });
  const withEmail = (scan.jobs || []).find((j) => j.contacts?.emails?.length);
  const pick = withEmail || (scan.jobs || [])[0];
  if (!pick) {
    const err = new Error('No WhatsApp jobs found to apply to. Scan exports first or pass jobText/recipientEmail.');
    err.code = 'CV_NO_JOB';
    throw err;
  }
  return {
...
function formatWhatsappCvSubmitSummary(mcpResult) {
  ...
  return [
    ...
    app.contacts?.emails?.[0] ? `To: ${app.contacts.emails[0]}` : 'To: (no email)',
...
async function streamWhatsappCvSubmitViaMcp(res, clientId, waCv, signal) {
  ...
  const mcpArgs = await resolveCvSubmitArgs(waCv, {
    onLog: (line) => {
      console.log(line);
      if (/\[mcp\]/i.test(line)) { const chunk = `${line}\n`; full += chunk; sendSse(res, 'token', { text: chunk }); }
    },
  });
  sendSse(res, 'tool_call', { tool: toolName, arguments: { ...mcpArgs, jobText: mcpArgs.jobText ? `${String(mcpArgs.jobText).slice(0, 120)}…` : undefined } });
  ...
  sendSse(res, 'tool_result', { tool: toolName, ok: true, status: mcpResult.application?.status, applicationId: mcpResult.application?.id });
  const summary = `\n${formatWhatsappCvSubmitSummary(mcpResult)}`;
  full += summary;
  sendSse(res, 'token', { text: summary });
  sendSse(res, 'done', { result: full, clientId, mcpTool: toolName, application: mcpResult.application });
}
