...
/**
 * Draft a CV application for a WhatsApp-discovered job.
 * @returns {{ ok: true, application: object, files: object }}
 */
export function submitWhatsappJobCv({
  ...
  if (!text && !recipientEmail) {
    const err = new Error(
      'submit_whatsapp_job_cv requires jobText or recipientEmail'
    );
    err.code = 'CV_INVALID_ARGS';
    throw err;
  }
  ...
  const cvCopyPath = path.join(
    dir,
    `${baseName}-cv${path.extname(attachedCv) || '.bin'}`
  );
