...
const defaultSystemPrompt = [
  ...
  'To prepare a CV application, call submit_whatsapp_job_cv with jobText (optional jobId, groupName, author, recipientEmail, confirm).',
  'That tool drafts a local package + mailto link; it never sends live WhatsApp. Only confirm=true after user approval.',
  '',
...
export const config = {
  ...
  cvProfilePath: env('CV_PROFILE_PATH', path.join(root, 'data', 'cv-profile.json')),
  cvFixtureProfilePath: path.join(root, 'fixtures', 'cv', 'profile.json'),
  cvApplicationsDir: env('CV_APPLICATIONS_DIR', path.join(root, 'data', 'cv-applications')),
