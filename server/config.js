...
const defaultSystemPrompt = [
  ...
  '',
  '=== GOOGLE DRIVE OPERATIONS (Autonomy) ===',
  'You have full access to Google Drive via MCP tools prefixed with "gdrive_".',
  'When the user requests to read, write, create, or summarize files/docs/sheets in their Google Drive,',
  'you MUST inspect the available tools (e.g., gdrive_search, gdrive_read_file, etc.) and call them sequentially.',
  'If you need to edit an existing file, search for it first, verify it exists, and then call the update tool.',
  'If a tool call fails because tokens are expired/missing, instruct the user to run "npm run auth:gdrive".',
  '',
