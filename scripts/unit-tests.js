...
test('submitWhatsappJobCv drafts package with mailto', () => {
  ...
  try {
    const result = submitWhatsappJobCv({
      jobId: 'test-job-1',
      jobText: "We're hiring a Backend Engineer. Send resume to mike@acme.dev",
...
test('executeMcpTool submit_whatsapp_job_cv uses fixture profile', async () => {
  ...
  const result = await executeMcpTool(
    ...
    { jobText: 'דרוש Full Stack. שלחו קו"ח ל-jobs@example.com', groupName: 'Jobs Israel', author: 'Dana HR' },
    { onLog: (line) => logs.push(line) }
  );
  assert.ok(result.ok);
