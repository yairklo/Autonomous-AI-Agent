...
- Track WhatsApp groups by **JID** as the ingest key. Display names are labels only; `getChat` per message often fails and stores `120363…@g.us`. On session ready, `getChats` binds each unique tracked name to its JID. Match `chatId` first.
